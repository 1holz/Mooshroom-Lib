...
public class Button extends WButton implements AdvTooltip {
    ...
    public Identifier setTexture(int mouseX, int mouseY) {
        String state = !isEnabled() ? "disabled"
                : (mouseX >= 0 && mouseY >= 0 && mouseX < width && mouseY < height) ? "hovered" : "regular";
    ...
    public void draw(MatrixStack matrices, int x, int y, int mouseX, int mouseY) {
        ...
        ScreenDrawing.texturedRect(matrices, x, y, widthFloor, heightFloor, texture, 0F, 0F, (float) widthFloor / max,
                (float) heightFloor / max, tint);
        ScreenDrawing.texturedRect(matrices, x + widthFloor, y, widthCeil, heightFloor, texture,
                1F - (float) heightCeil / max, 0F, 1F, (float) widthFloor / max, tint);
        ScreenDrawing.texturedRect(matrices, x, y + heightFloor, widthFloor, heightCeil, texture, 0F,
                1F - (float) widthCeil / max, (float) heightFloor / max, 1F, tint);
        ScreenDrawing.texturedRect(matrices, x + widthFloor, y + heightFloor, widthCeil, heightCeil, texture,
                1F - (float) heightCeil / max, 1F - (float) widthCeil / max, 1F, 1F, tint);
        if (overlay != null)
            overlay.paint(matrices, x, y, mouseX, mouseY);
        if (getLabel() != null) {
            int color = isEnabled() ? 0xE0E0E0 : 0xA0A0A0;
            ScreenDrawing.drawStringWithShadow(matrices, getLabel().getString(), alignment, x, y + ((20 - 8) / 2),
                    width, color);
        }
    }
